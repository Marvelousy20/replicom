...
export async function GET(request: NextRequest, res: NextResponse) {
    ...
    try {
        const response = await axios.get(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/registeruser/?walletAddress=${walletAddress}`);
        console.log(response.data.length, response.data, "-response")
        if (response.data.length == 0) {
            const res = await axios.post(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/registeruser/`, { walletAddress: `${walletAddress}` })
            const response = await axios.get(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/registeruser/?walletAddress=${walletAddress}`);
