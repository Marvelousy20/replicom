...
const Prediction = () => {
    ...
    async function getPredictionData() {
        const response = await axios.get(`http://127.0.0.1:8000/api/prediction/?walletAddress=${walletAddress}`)
                ...
                <tbody>
                    {
                        predictionData.map((data, key) => (
                            <tr className="border-b-[1px] border-gray-600" key={key}>
                                ...
                                <td className="p-[10px]">{moment(data.created).format("YYYY-MM-DD hh:mm:ss")}</td>
                            </tr>)
